...
<div class="wrapper">
  ...
  <section class="recap__part">
    <h3 class="section__title">Inventaires</h3>

    @foreach($commande->event->inventaires as $inventaire)
      <div class="recap__inventaire">
        <h4 class="recap__title">Inventaire du {{$inventaire->date}}</h4>
        @if($inventaire->responsable)
          <p class="recap__lign"><span class="element__nom">Représentant AGEL :</span>{{$inventaire->responsable->prenom}} {{$inventaire->responsable->nom}}</p>
        @else
          <p class="recap__lign">Représentant AGEL non attribué actuellement.</p>
        @endif
      </div>
    @endforeach
  </section>
  @else
    <div class="title__container">
      <h2 class="page__title">Votre (N°{{$commande->id}}) est en cours de traitement</h2>
